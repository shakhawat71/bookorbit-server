...
app.patch("/users/role/:email", verifyToken, requireRole(["admin"]), async (req, res) => {
  try {
...
app.get("/books/mine", verifyToken, requireRole(["librarian", "admin"]), async (req, res) => {
  try {
    const email = normalizeEmail(req.user.email);
...
app.get("/admin/books", verifyToken, requireRole(["admin"]), async (req, res) => {
  try {
...
app.patch("/admin/books/:id/status", verifyToken, requireRole(["admin"]), async (req, res) => {
  try {
    const { id } = req.params;
...
app.get("/reviews/eligible/:bookId", verifyToken, async (req, res) => {
  try {
    ...
    res.status(500).send({ eligible: false, reason: "Server error" });
  }
});
...
app.post("/reviews", verifyToken, async (req, res) => {
  try {
    ...
    if (!hasOrder) {
      return res.status(403).send({ message: "You must order this book to review it." });
    }
...
// =====================================================
app.get("/librarian/orders", verifyToken, requireRole(["librarian", "admin"]), async (req, res) => {
  try {
    const librarianEmail = normalizeEmail(req.user.email);
    const role = await getUserRole(librarianEmail);
...
app.patch("/librarian/orders/:id/status", verifyToken, requireRole(["librarian", "admin"]), async (req, res) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
...
app.patch("/librarian/orders/:id/cancel", verifyToken, requireRole(["librarian", "admin"]), async (req, res) => {
  try {
    const { id } = req.params;
